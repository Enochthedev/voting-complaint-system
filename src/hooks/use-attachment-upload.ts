...
export function useAttachmentUpload(): UseAttachmentUploadReturn {
   ...
   */
  const uploadFiles = useCallback(async (files: File[], complaintId: string, userId: string) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        // Upload the file
        const result = await uploadAttachment(file, complaintId, userId, (progress) => {
          setUploadProgress((prev) =>
  ...
  const removeAttachment = useCallback(
    async (attachmentId: string, filePath: string): Promise<boolean> => {
      ...
      if (result.success) {
        ...
        setUploadProgress((prev) => prev.filter((item) => item.attachment?.id !== attachmentId));
        return true;
      }
