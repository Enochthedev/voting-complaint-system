...
describe('HighlightText', () => {
  it('renders plain text when no query is provided', () => {
    const { container } = render(<HighlightText text="Broken AC in Lecture Hall" />);
  ...
  it('handles empty query gracefully', () => {
    const { container } = render(<HighlightText text="Broken AC in Lecture Hall" query="" />);
  ...
  it('applies custom className', () => {
    const { container } = render(
      <HighlightText text="Test text" query="test" className="custom-class" />
    );
  ...
  it('applies custom highlight className', () => {
    const { container } = render(
      <HighlightText text="Test text" query="test" highlightClassName="custom-highlight" />
    );
  ...
  it('handles special regex characters in query', () => {
    const { container } = render(<HighlightText text="Cost is $100 (approx.)" query="$100" />);
...
describe('HighlightHTML', () => {
  it('strips HTML tags before highlighting', () => {
    const { container } = render(
      <HighlightHTML html="<p>Broken <strong>AC</strong> in Lecture Hall</p>" query="lecture" />
    );
  ...
  it('handles HTML entities', () => {
    const { container } = render(<HighlightHTML html="<p>Cost &gt; $100</p>" query="cost" />);
