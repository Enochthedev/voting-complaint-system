...
export function Loading({ size = 'md', text, className, fullScreen = false }: LoadingProps) {
  ...
  const content = (
    <div className={cn('text-center', className)}>
      <Loader2
        className={cn('animate-spin text-foreground', sizeClasses[size], text ? 'mx-auto' : '')}
      />
...
export function LoadingSpinner({
  ...
  return <Loader2 className={cn('animate-spin text-foreground', sizeClasses[size], className)} />;
}
