...
```javascript
const { data, error } = await supabase.from('escalation_rules').select('*').eq('is_active', true);
```
...
```javascript
const { data, error } = await supabase.from('escalation_rules').delete().eq('id', ruleId);
```
