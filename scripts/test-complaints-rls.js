...
async function testRLSPolicies() {
  ...
  try {
    ...
    const requiredPolicies = [
      {
        name: 'Students view own complaints',
        cmd: 'SELECT',
        description: 'Students can view their own complaints',
      },
      {
        name: 'Students insert complaints',
        cmd: 'INSERT',
        description: 'Students can insert new complaints',
      },
      {
        name: 'Students update own drafts',
        cmd: 'UPDATE',
        description: 'Students can update their draft complaints',
      },
      {
        name: 'Lecturers update complaints',
        cmd: 'UPDATE',
        description: 'Lecturers can update all complaints',
      },
      {
        name: 'Students delete own drafts',
        cmd: 'DELETE',
        description: 'Students can delete their draft complaints',
      },
    ];

    for (const required of requiredPolicies) {
      const found = policies.find((p) => p.policyname === required.name && p.cmd === required.cmd);
    ...
    try {
      const { error: anonError } = await adminClient.from('complaints').insert({
        student_id: null,
        is_anonymous: true,
        is_draft: false,
      ...
      if (!anonError) {
        ...
        // Clean up
        await adminClient.from('complaints').delete().eq('title', 'RLS Test Anonymous Complaint');
      } else {
    ...
    try {
      const { error: draftError } = await adminClient.from('complaints').insert({
        student_id: null,
        is_anonymous: true,
        is_draft: true,
      ...
      if (!draftError) {
        ...
        // Clean up
        await adminClient.from('complaints').delete().eq('title', 'RLS Test Draft Complaint');
      } else {
