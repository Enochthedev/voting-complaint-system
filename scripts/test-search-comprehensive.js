...
async function testSearch(testName, searchQuery, expectedMinResults = 0) {
  ...
  console.log(
    `   ${passed ? '✅' : '❌'} Found ${foundCount} result(s) (expected at least ${expectedMinResults})`
  );

  if (foundCount > 0 && foundCount <= 3) {
    data
      .filter((c) => insertedIds.includes(c.id))
      .forEach((result) => {
...
async function runTests() {
  ...
  try {
    ...
    result = await testSearch(
      'Test 6.2: Search for "projector OR computer"',
